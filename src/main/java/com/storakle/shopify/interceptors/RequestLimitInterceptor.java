...
public class RequestLimitInterceptor implements RequestInterceptor {
  ...
  public void apply(RequestTemplate template) {
    ...
    while (tryGetCredit) {
      // Lock per shopify store. The lock is distributed, so it will work for multiple threads and
      // applications.
      RLock lock = redisson.getLock(_shopifyRedissonManager.getMyShopifyUrl());

      RAtomicLong isDefaultRemainingCreditsValueSet =
          redisson.getAtomicLong(_shopifyRedissonManager.getIsDefaultRemainingCreditsValueSetKey());

      RAtomicLong remainingCreditsAtomic =
          redisson.getAtomicLong(_shopifyRedissonManager.getRemainingCreditsKey());
      ...
      RAtomicLong lastRequestTimeAtomic =
          redisson.getAtomicLong(_shopifyRedissonManager.getLastRequestTimeKey());
      Long remainingCredits = remainingCreditsAtomic.get();

      if (remainingCredits > 0) {
        // These values are set here, because a request can be made while the current request is
        // still in progress.
        // We set the actual values inside the decoder (when the request is complete), but if we
        // don't set them here
        ...
        // If the latest request's remaining calls were 0 and no calls were made after that, the
        // remaining credits
        // will not be updated. This is why the last request time is used as well.
        long availableCalls =
            (long) Math.floor((System.currentTimeMillis() - lastRequestTimeAtomic.get()) / 500);
        ...
        if (availableCalls > 0) {
          ...
          try {
            ...
            System.out.println(
                "Error while waiting for available Shopify call credit. " + e.getMessage());
          }
        }
      }
    }
